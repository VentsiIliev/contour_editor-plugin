...
def test_add_segment_execute(mock_manager, mock_event_bus):
    ...
    from contour_editor.commands.segment_commands import AddSegmentCommand
    new_segment = Mock()
    ...
    with patch('contour_editor.commands.segment_commands.EventBus.get_instance', return_value=mock_event_bus):
        cmd.execute()
...
def test_delete_segment_execute(mock_manager, mock_event_bus, mock_segment_with_layer):
    ...
    from contour_editor.commands.segment_commands import DeleteSegmentCommand
...
def test_toggle_visibility_execute(mock_manager, mock_event_bus, mock_segment_with_layer):
    ...
    from contour_editor.commands.segment_commands import ToggleSegmentVisibilityCommand
    mock_manager.segments = [mock_segment_with_layer]
...
def test_change_layer_execute(mock_manager, mock_event_bus, mock_segment_with_layer):
    ...
    from contour_editor.commands.segment_commands import ChangeSegmentLayerCommand
    mock_manager.segments = [mock_segment_with_layer]
