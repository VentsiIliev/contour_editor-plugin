...
def test_get_main_contour_points(contour_service, mock_manager):
    ...
    result = contour_service.get_main_contour_points()
    assert result is not None
    assert isinstance(result, np.ndarray)
    assert result.shape == (4, 2)
    assert result[0, 0] == 0.0
    assert result[0, 1] == 0.0
def test_get_main_contour_points_no_workpiece(contour_service, mock_manager):
    """Test when no Main segment exists - should fallback to first segment."""
    seg = Mock()
    seg.layer = Mock(name="Contour")
    seg.layer.name = "Contour"
    seg.points = [QPointF(0, 0), QPointF(100, 0), QPointF(100, 100), QPointF(0, 100)]
    mock_manager.get_segments = Mock(return_value=[seg])
    result = contour_service.get_main_contour_points()
    # Should fallback to first available segment
    assert result is not None
    assert len(result) == 4
def test_get_main_contour_points_insufficient_points(contour_service, mock_manager):
