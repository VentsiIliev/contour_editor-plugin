...
class BottomToolBar(QWidget):
    ...
    def setupUI(self):
        ...
        self.zoom_out_button = self.create_button(icon_provider.get_icon('zoom_out'), self.zoom_out_requested.emit, "Zoom Out", "zoom_out")
        self.reset_zoom_button = self.create_button(icon_provider.get_icon('reset_zoom'), self.reset_zoom_requested.emit, "Reset Zoom",
                                                    "reset_zoom")
        self.zoom_in_button = self.create_button(icon_provider.get_icon('zoom_in'), self.zoom_in_requested.emit, "Zoom In", "zoom_in")

        # Create pan/edit toggle button
        self.pan_toggle_button = self.create_button(icon_provider.get_icon('drag'), self.toggle_pan_mode, "Toggle Pan/Edit Mode",
                                                    "pan_toggle")
    ...
    def create_button(self, icon, click_handler, tooltip, button_type):
        button = QPushButton()

        # Set up icon and text
        # icon can be a QIcon or a QPixmap
        if isinstance(icon, QIcon):
            if not icon.isNull():
                button.setIcon(icon)
                button.setIconSize(QSize(32, 32))
            else:
                # Fallback text if icon is null
            ...
            # Fallback text for any other case
            if button_type == "zoom_in":
                button.setText("+")
            elif button_type == "zoom_out":
                button.setText("-")
    ...
    def toggle_pan_mode(self):
        ...
        if self.is_drag_mode:
            pointer_icon = icon_provider.get_icon('pointer')
            if not pointer_icon.isNull():
                self.pan_toggle_button.setIcon(pointer_icon)
            else:
            ...
            drag_icon = icon_provider.get_icon('drag')
            if not drag_icon.isNull():
                self.pan_toggle_button.setIcon(drag_icon)
            else:
