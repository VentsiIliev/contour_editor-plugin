...
class TopBarWidget(QWidget):
    ...
    def __init__(self, contour_editor=None, point_manager=None):
        ...
        self.undo_button = self.create_button(icon_provider.get_icon('undo'), self.undo_requested.emit)
        self.redo_button = self.create_button(icon_provider.get_icon('redo'), self.redo_requested.emit)
        ...
        self.remove_button = self.create_button(icon_provider.get_icon('remove'), self.dummy_remove_action)
        self.remove_button.setToolTip("Remove selected point(s)\nLong press to enter multi-selection mode")
        self.capture_image_button = self.create_button(icon_provider.get_icon('CAPTURE_IMAGE'), self.on_capture_image)

        self.settings_button = self.create_button(icon_provider.get_icon('SETTINGS_BUTTON'), self.settings_requested.emit)
        self.tools_button = self.create_button(icon_provider.get_icon('TOOLS'), self.tools_requested.emit)

        # Pickup point
        self.pickup_point_button = self.create_button(IconProvider.get().get_icon('pickup_point'), None, 'Pickup')
        self.pickup_point_button.setCheckable(True)

        self.preview_path_button = self.create_button(icon_provider.get_icon('preview'), self.preview_requested.emit)

        self.zigzag_button = self.create_button(icon_provider.get_icon('zigzag'), self.zigzag_requested.emit)
        self.offset_button = self.create_button(icon_provider.get_icon('offset'),self.offset_requested.emit)
        ...
        self.startButton = self.create_button(icon_provider.get_icon('START_BUTTON'), self.start_requested.emit,"")
        self.save_button = self.create_button(icon_provider.get_icon('SAVE_BUTTON'), self.save_requested.emit)
        ...
        self.buttons = [self.capture_image_button, self.settings_button, self.tools_button,
                        self.remove_button, self.pickup_point_button, self.preview_path_button,
                        self.zigzag_button, self.offset_button, self.undo_button,
                        self.redo_button, self.save_button, self.startButton]
    ...
    def create_button(self, icon_path, click_handler=None, text=None):
        button = QPushButton("")
        if icon_path:
    ...
    def update_multi_select_ui(self, is_active):
        """Update UI to reflect a multi-select mode state (called from handler)"""
